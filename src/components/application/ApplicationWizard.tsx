...
type Step = "learn" | "check" | "decide" | "consent" | "profile" | "review" | "complete" | "declined";
type PilotConfig = {
  mode: "synthetic" | "live";
...
export function ApplicationWizard() {
  ...
  async function submit() {
    ...
    try {
      ...
      setReference(result.applicationRef); setProfile(emptyProfile); window.sessionStorage.removeItem("safecard-case-id"); setStep("complete");
  ...
  const labels = locale === "fil" ? ["Matuto", "Suriin", "Magpasya", "Pahintulot", "Form", "Review"] : ["Learn", "Check", "Decide", "Consent", "Form", "Review"];
  const progress = ["learn", "check", "decide", "consent", "profile", "review"].indexOf(step);
  ...
  return <main className="wizard-page">
    ...
    <section className="wizard-card">
      ...
      {step === "review" && <><p className="eyebrow">06 · Review</p><h1>{locale === "fil" ? "Suriin bago isumite." : "Review before submitting."}</h1><dl className="review-list"><div><dt>Name</dt><dd>{profile.first_name} {profile.last_name}</dd></div><div><dt>Date of birth</dt><dd>{profile.date_of_birth}</dd></div><div><dt>Mobile</dt><dd>{profile.mobile_number}</dd></div><div><dt>Address</dt><dd>{profile.address_line1}, {profile.city}, {profile.province} {profile.zip_code}</dd></div></dl><div className="parked-panel"><strong>Payment provider parked</strong><p>No charge is taken in this build. Official payment handoff can be enabled only after PRC approves the route and configuration. Submission and payment remain separate states.</p></div>{error && <p className="form-message error">{error}</p>}<div className="wizard-actions"><button className="button-primary" disabled={busy} onClick={submit}>{busy ? "Submitting…" : isLive ? "Submit application →" : "Complete demo →"}</button><button className="button-quiet" onClick={() => setStep("profile")}>Edit</button></div></>}
