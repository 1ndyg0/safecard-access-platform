...
export function ManualPaymentPanel({ caseId, campaignId, live, onComplete }: { caseId: string; campaignId: string; live: boolean; onComplete: () => void }) {
  ...
  const ui = isFil ? {
    unavailable: "Hindi available ang payment configuration.", chooseRoute: "Pumili ng approved payment route.", required: "Pumili ng route, ilagay ang petsa at eksaktong halaga, idagdag ang payment reference, mag-upload ng proof, at kumpirmahin ang payer declaration.", handoffError: "Hindi mabuksan ang payment handoff.", referenceError: "Hindi ma-record ang payment reference.", uploadError: "Hindi na-upload ang proof.", completeError: "Hindi nakumpleto ang payment.", received: "Natanggap ang proof. Naka-pending ang staff verification; hindi pa active ang membership.", loading: "Nilo-load ang approved payment routes…", eyebrow: "07 · Bayad at proof", title: "Magbayad sa labas ng SafeCard, pagkatapos ay bumalik kasama ang proof.", fee: "Membership fee:", monthly: "Ang monthly equivalent ay paliwanag lamang at hindi installment offer.", separateTitle: "Hiwalay na state ang payment", separateBody: "Ang payment ay hindi consent, application submission, approval, PRC handoff, o membership activation. PRC lamang ang makakapagkumpirma.", warning: "Babala para sa controlled pilot", parked: "Hindi pa naka-enable ang official payment handoff.", routeLegend: "Pumili ng payment route", accountName: "Account name", bank: "Bank", accountNumber: "Account number", swift: "SWIFT", branch: "Branch", transferHelp: "Kumpletuhin ang transfer sa bank o GCash app. Hindi humahawak, naglilipat, o nagsi-settle ng funds ang SafeCard.", reference: "Payment reference o transaction number", referencePlaceholder: "Ilagay ang reference mula sa bank o GCash", paidDate: "Petsa ng bayad", amountPaid: "Halagang binayaran (PHP)", proof: "Proof of payment (JPEG, PNG, WebP, o PDF; max 10 MB)", preview: "Preview ng napiling payment proof", declaration: "Kinukumpirma kong ginawa ang transfer sa labas ng SafeCard at naiintindihan kong hindi ina-activate ng verification ang membership.", uploading: "Secure na ina-upload…", submit: "Isumite ang proof para ma-review →", copied: "Na-copy na", recorded: "Naitala ang payment reference. Ire-review ng staff ang proof; mananatiling inactive ang membership hanggang kumpirmahin ng PRC.", qrAlt: "Official GCash QR code"
  } : {
    unavailable: "Payment configuration is unavailable.", chooseRoute: "Choose an approved payment route.", required: "Choose a route, enter the payment date and exact amount, add the reference, upload proof, and confirm the payer declaration.", handoffError: "Payment handoff could not be opened.", referenceError: "Payment reference could not be recorded.", uploadError: "Proof upload failed.", completeError: "Payment could not be completed.", received: "Proof received. Staff verification is pending; membership is not active.", loading: "Loading approved payment routes…", eyebrow: "07 · Payment and proof", title: "Pay outside SafeCard, then return with proof.", fee: "Membership fee:", monthly: "The monthly equivalent is only explanatory and is not an installment offer.", separateTitle: "Payment is a separate state", separateBody: "Payment does not equal consent, application submission, approval, PRC handoff, or membership activation. Only PRC confirmation can activate membership.", warning: "Controlled pilot warning", parked: "Official payment handoff is not yet enabled.", routeLegend: "Choose a payment route", accountName: "Account name", bank: "Bank", accountNumber: "Account number", swift: "SWIFT", branch: "Branch", transferHelp: "Complete the transfer in your bank or GCash app. SafeCard does not hold, move, or settle funds.", reference: "Payment reference or transaction number", referencePlaceholder: "Enter the reference shown by your bank or GCash", paidDate: "Date paid", amountPaid: "Amount paid (PHP)", proof: "Proof of payment (JPEG, PNG, WebP, or PDF; max 10 MB)", preview: "Selected payment proof preview", declaration: "I confirm this transfer was completed outside SafeCard and understand that verification does not activate membership.", uploading: "Uploading securely…", submit: "Submit proof for review →", copied: "Copied", recorded: "Payment reference recorded. Staff will review the proof; your membership remains inactive until PRC confirms it.", qrAlt: "Official GCash QR code"
  };
  ...
  useEffect(() => {
    fetch(`/api/payment-config?campaign_id=${encodeURIComponent(campaignId)}`, { cache: "no-store" }).then((response) => response.json()).then((data: PaymentConfig) => { setConfig(data); if (data.amount) setAmountPaid(String(data.amount)); }).catch(() => setConfig({ available: false, reason: "Payment configuration is unavailable.", amount: null, currency: "PHP", routes: [] }));
  }, [campaignId]);
  ...
  async function submitPayment() {
    ...
    if (!reference.trim() || !paidDate || !file || !declaration || Number(amountPaid) !== config.amount) { setError(ui.required); return; }
    setBusy(true);
    try {
  ...
  return <div className="payment-panel">
    ...
    {config.available && <>
      <fieldset className="payment-route-list"><legend>{ui.routeLegend}</legend>{config.routes.map((route) => <label className={`payment-route ${route.id === routeId ? "selected" : ""}`} key={route.id}><input type="radio" name="payment-route" value={route.id} checked={route.id === routeId} disabled={Boolean(paymentIntentId)} onChange={() => setRouteId(route.id)} /><span><strong>{route.label}</strong><small>{route.instructions}</small></span></label>)}</fieldset>
      ...
      <div className="form-grid"><label className="field-block"><span>{ui.paidDate}</span><input type="date" value={paidDate} max={new Date().toISOString().slice(0, 10)} onChange={(event) => setPaidDate(event.target.value)} required /></label><label className="field-block"><span>{ui.amountPaid}</span><input type="number" min="0.01" step="0.01" inputMode="decimal" value={amountPaid} onChange={(event) => setAmountPaid(event.target.value)} required /></label></div>
      <label className="upload-field"><span>{ui.proof}</span><input type="file" accept="image/jpeg,image/png,image/webp,application/pdf" onChange={(event) => setFile(event.target.files?.[0] ?? null)} />{previewUrl && file?.type !== "application/pdf" && <Image src={previewUrl} alt={ui.preview} className="receipt-preview" width={720} height={960} unoptimized />}{previewUrl && file?.type === "application/pdf" && <iframe src={previewUrl} title={ui.preview} className="receipt-preview" sandbox="" referrerPolicy="no-referrer" />}</label>
