...
async function createStaff(admin: SupabaseClient, email: string, fullName: string) {
  ...
  if (existing) {
    ...
    await admin.from('users').upsert({ id: existing.id, email, full_name: fullName }, { onConflict: 'id' });
...
export async function seedWorld(): Promise<SeededWorld> {
  ...
  await admin
    .from('organizations')
    .insert({ id: organizationId, name: 'E2E School', slug: `e2e-school-${organizationId}`, org_type: 'school' });
